...
class RdPlatform:
    def __init__(self, is_cluster=False):
        ...
        self.ip = cfg.G_CONFIG_DICT['base.url_base'].split('//')[1]
        self.url_base = 'http://' + cfg.G_CONFIG_DICT['rd_platform.host']
        self.user_email = cfg.G_CONFIG_DICT['rd_platform.user_email']
    ...
    def restart_service(self, service_name):
        if self.is_cluster:
            return self.recreate_cluster_service(service_name)
        else:
    ...
    def get_service_version(self, service_name):
        env_list = self.__get_service_list()
        for env in env_list:
            if env['service'] == service_name:
                return env['version'].split('.')[1]
...
if __name__ == '__main__':
    ...
    # print(rdp.cluster_service_dict)
    rdp = RdPlatform()
    version = rdp.get_service_version('wareflipagv')
    print(version)
